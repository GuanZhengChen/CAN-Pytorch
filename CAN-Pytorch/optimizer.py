...
def weighted_cross_entropy_with_logits(logits, targets, pos_weight):
    return targets * -torch.log(F.sigmoid(logits)) * pos_weight + (1 - targets) * -torch.log(1 - F.sigmoid(logits))

class OptimizerCAN(object):

    def __init__(self, preds, labels, model, num_nodes, num_features, pos_weight_u, norm_u, pos_weight_a, norm_a,learning_rate):
        preds_sub_u, preds_sub_a = preds
        labels_sub_u, labels_sub_a = labels
        self.cost_u = norm_u * tf.reduce_mean(weighted_cross_entropy_with_logits(logits=preds_sub_u, targets=labels_sub_u, pos_weight=pos_weight_u))
        self.cost_a = norm_a * tf.reduce_mean(weighted_cross_entropy_with_logits(logits=preds_sub_a, targets=labels_sub_a, pos_weight=pos_weight_a))
